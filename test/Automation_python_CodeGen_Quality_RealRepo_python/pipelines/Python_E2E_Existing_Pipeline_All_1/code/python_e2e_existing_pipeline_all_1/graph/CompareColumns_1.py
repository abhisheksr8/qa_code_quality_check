...
def CompareColumns_1(spark: SparkSession, in0: DataFrame, in1: DataFrame) -> DataFrame:
    ...
    for vColumn in set(in0.columns).difference({"first_name_new1"}):
    ...
    return df1\
        ...
        .join(
          ...
          reduce(
            lambda a, c: a & c,
            [col("exploded1.column_name") == col("exploded2.column_name"),              col("exploded1.first_name_new1") == col("exploded2.first_name_new1")],
            lit(True)
          ),
          "full_outer"
        )\
        .select(
          ...
          coalesce(col("exploded1.first_name_new1"), col("exploded2.first_name_new1")).alias("first_name_new1"), 
          col(
              "exploded1.##value##"
            )\
            .alias(
        ...
        .union(
          df3\
            ...
            .withColumn(
              "##row_number###",
              row_number()\
                .over(Window.partitionBy(col("column_name"), col("first_name_new1")).orderBy(col("first_name_new1")))
            )\
            .filter(
              (
                col("##row_number###")
        ...
        .agg(
          ...
          first(
              when(
                  coalesce(col("mismatch_example_left"), col("mismatch_example_right")).isNotNull(),
                  col("first_name_new1")
                )\
                .otherwise(lit(None)),
              ignorenulls = True
            )\
            .alias("mismatch_example_first_name_new1")
        )\
